"""apt2 独立精修层：读取布局副本，不反向修改 apt1 的共享声明。"""
...
def apply(source):
    ...
    for key, r in layout.ROOMS.items():
        x0, y0, x1, y1 = r["rect"]
        if "bath" in key or key in ("powder", "laundry", "primary_closet"):
            width = min(1.5, (x1 - x0) - 0.7)
            furniture += b.cabinet(
                key + "_vanity", key, (x0 + x1) / 2, y0 + 0.65, width, 0.50, 0.82
            )
            if "bath" in key:
                furniture.append(
                    b.box(
    ...
    for name, x, y, sx, sy, top, mat in source["SKYLINE"]:
        key = (mat, int(x // district_size), int(y // district_size))
        batch = city_batches.setdefault(key, dict(vertex=[], texcoord=[], face=[]))
        height = top + source["ELEV"]
        shape = rounded_mesh((sx, sy, height), radius=0)
        offset = len(batch["vertex"])
        batch["vertex"] += [
            (vx + x, vy + y, vz - source["ELEV"] + height / 2) for vx, vy, vz in shape["vertex"]
        ]
        batch["texcoord"] += [(i % 2, (i // 2) % 2) for i in range(len(shape["vertex"]))]
        batch["face"] += [tuple(offset + k for k in face) for face in shape["face"]]
    city_geoms = []
    for i, ((mat, *_), mesh) in enumerate(city_batches.items()):
        name = f"a2_city_batch{i}"
        b.meshes[name] = mesh
        city_geoms.append(
            dict(name=name, type="mesh", mesh_name=name, pos=(0, 0, 0), mat=mat, collide=False)
        )
    result.update(
        ROOMS=layout.ROOMS,
        FURNITURE=furniture,
