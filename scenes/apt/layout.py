"""apt —— 曼哈顿 62–63 层复式，含四时段环境和真实家具交互。

城市坐标、天空、航拍和时段颜色来自 scenes.environments.manhattan。
不要为迁移重新下载 NYC 数据；保存的城市建筑坐标与标高必须保持一致。

════════════════════════════════════════════════════════════════════════
⭐ 竖向尺寸是**算出来的**，而且推导方向和 house 相反 —— 这一点必须写明白
════════════════════════════════════════════════════════════════════════

house 的规矩是「先定台阶 → 推层高」。apt 有一个外部硬约束把方向掉了个个儿：

    **层高必须等于 环境的 FLOOR_TO_FLOOR = 3.75 m。**
...
⛔ **不能抄 house 的 0.16 / 9 踢面 / 2.88 层高** —— 那套是它自己反推出来的。
"""
...
from scenes.environments import manhattan as _ENV
...
WINDOW_SILL_H = 0.95                     # 普通窗（apt 全是落地窗，这两个只为满足契约）
WINDOW_TOP_H = 2.30

# ── 这栋楼在哪一层：⚠️ 三个数必须和环境定义逐位一致，外景才对得上 ──────────
FLOOR_LEVEL = _ENV.FLOOR_LEVEL            # 62
FLOOR_TO_FLOOR = _ENV.FLOOR_TO_FLOOR      # 3.75
ELEV = _ENV.ELEV                          # 232.5
assert FLOOR_TO_FLOOR == 3.75 and abs(ELEV - FLOOR_LEVEL * FLOOR_TO_FLOOR) < 1e-9, \
    "层高/标高和环境定义对不上，窗外那片城市会整体错位"
...
WALL_HEIGHT = STOREY_H - CEILING_THICK - FLOOR_THICK     # 3.60（扣除顶板和地板）
...
# ────────────────────────────────────────────────────────── 平面网格
# ⭐ 外轮廓与本楼外轮廓一致（23.0 × 15.0 m）——这样 `HOST_TOWER` 的立面尺寸、
...
#    和隔壁屋的楼板严丝合缝接上；house 第一版取净空边，门槛底下留了 0.14 m 裂缝。
#    只有东边取净空里的梯段起跑线。
...
def _glass_wall(room: str, side: str, a: float, b: float, n: int, *,
    ...
    ⚠️ 这个函数是**从 apt1 抄过来的，有意不共用**：它闭包着本场景自己的
       `FLOOR_WINDOW_SILL / FLOOR_WINDOW_TOP`（apt 层高更高，窗顶到 3.48 而不是 3.18）。
       ⛔ 别为了"去重"把它提到 `scenes/furniture.py` 里——那会把 apt 的窗高
...
GLASS_RGBA = _ENV.GLASS_RGBA
GLASS_THICK = _ENV.GLASS_THICK
WALL_FACE_FIX = True
...
WALL_ARTS = [
    {"room": "gallery", "side": "n", "center": -3.60, "z": 1.75, "w": 1.80, "h": 1.25, "tex": "art0"},
    {"room": "gallery", "side": "n", "center": -1.15, "z": 1.75, "w": 1.40, "h": 1.10, "tex": "art2"},
    {"room": "gallery", "side": "s", "center": -0.80, "z": 1.75, "w": 1.60, "h": 1.15, "tex": "art1"},
    {"room": "gallery", "side": "s", "center": 4.60, "z": 1.75, "w": 0.95, "h": 1.05, "tex": "art3"},
    {"room": "great_room", "side": "e", "center": 2.65, "z": 1.80, "w": 1.70, "h": 1.20, "tex": "art2"},
    {"room": "dining", "side": "w", "center": 5.00, "z": 1.80, "w": 1.40, "h": 1.00, "tex": "art1"},
    {"room": "foyer", "side": "e", "center": -2.65, "z": 1.75, "w": 1.20, "h": 0.90, "tex": "art3"},
    {"room": "study", "side": "e", "center": 4.10, "z": 1.75, "w": 1.30, "h": 0.95, "tex": "art0"},
]

# ────────────────────────────────────────────────────────── 窗外（独立环境模块）
# ⛔ 一律**只读引用**，绝不重跑 make_view.py --nyc（见文件抬头）
SKYBOX = dict(_ENV.SKYBOX)
TEXTURES_EXTRA = list(_ENV.TEXTURES_EXTRA)
MATERIALS_EXTRA = list(_ENV.MATERIALS_EXTRA)
GROUND_SLABS = list(_ENV.GROUND_SLABS)
SKYLINE = list(_ENV.SKYLINE)
PARK_NEAR_Y, PARK_W, PARK_L, CITY_SPAN = _ENV.PARK_NEAR_Y, _ENV.PARK_W, _ENV.PARK_L, _ENV.CITY_SPAN

# 本楼的外皮：保留环境中的两件，另加两圈**环形**的立面构件。
#
...
# ⭐ 做成环、且**摆在外轮廓之外**（像 `host_ledge` 那样往外挑），
#    就永远不会伸进任何房间或梯井。
_TOP_Z = FLOOR_Z(1) + WALL_HEIGHT
_ENV_X, _ENV_Y = 23.0, 15.0          # 外轮廓（= X4−X0 / Y3−Y0，与本楼外轮廓一致）
...
# ────────────────────────────────────────────────────────── 灯光
# 最多 7 盏场景灯，另留一个渲染灯槽给相机补光。
# 生成器关闭机器人模型的默认摄影棚灯；检查编译后的实际活动灯数。
LIGHT_BUDGET = 7
LIGHTS: list[dict] = [
...
def stair_route(floor: int) -> list[tuple[float, float, float]]:
    ...
    house 上一版的 bug 正是"平台在一处、回头跑起点在另一处"，两处各写各的没人对得上。
    """
    base = FLOOR_Z(floor)
    return [
...
# 只在 apt 应用住宅精修；所有来自环境模块的嵌套声明先深复制。
from scenes.apt.refinement import apply as _refine_residence
GEOM_BANDS = _ENV.GEOM_BANDS
globals().update(_refine_residence(globals()))

from scenes.apt.interactivity import apply as _interactive_residence
globals().update(_interactive_residence(globals()))

from scenes.apt.time_presets import PHASES, presets as _time_presets
LIGHTS_BY_TIME = _time_presets(_ENV, globals())
