# apt2 furniture interaction

apt2 is the scene for developing household interaction. Its native walkthrough supports
...
Armchairs and coffee tables remain fixed. Cabinet fronts and island stools remain static.
...
![Open refrigerator](../images/apt2/interactions/fridge-open.png)
...
CC-BY-4.0 attribution. The original [decor lock](../../decor/decor.lock.json) and reference-scene
...
upgrading assets. The city and reference-scene textures are already tracked and need no
...
and the generated Go2 variant has 12. NERV currently registers the G1 variant of apt2.
...
Further development should use apt2 in this order:
...
3. Train and validate the required robot manipulation skills with a suitable hand model in the
   policy/training repositories. Validate contact, grasp release and failure recovery in apt2
...
python tools/make_time_stills.py --scene apt2
python tools/render_residences.py --scene apt2 --contract ../policies/g1-29dof-turn/contract.json
python tools/benchmark_residences.py --scene apt2 --seconds 30 --output temp/interactions/runtime
```
...
[Validation record](validation.md) · [Residence guide](../residences/README.md)
