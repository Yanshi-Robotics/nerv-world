...
目前有**四个地方**——一套单层大平层，一栋带楼梯的三层小楼，一套 62 层、窗外是中央公园的
...
| **apt2** | 曼哈顿复式顶层豪宅，**62–63 层**——双高大客厅 + 一部双跑楼梯 | 24 个空间 / 690 ㎡ | 为**「够得着」**建的。前三个场景里的家具全是焊死的实心块，机器人只能撞上去被挡住。这里餐椅/单椅/茶几走凸分解做**真碰撞**（椅腿之间真的能穿过去），沙发按 G1 腿长做成座面 0.35 m —— **机器人真的坐得下** |
...
前三个场景里，屋里每一件家具都焊死在世界上、零自由度，而且套了真网格的那些，
碰撞体是**一个把整件家具裹起来的实心盒**。机器人只能撞上去被挡住——推不动、拿不起、坐不下。

apt2 就是为了这件事建的。上面这张不是摆拍：机器人被摆成坐姿之后**推了 3 秒物理**
才拍的，骨盆只抬了 4.6 cm、水平滑了 3.1 cm、躯干倾 11.4°——它是真坐住了。
...
同一把餐椅，从上往下打一片射线量高度（单位 cm，`·` = 一直打到地面）：

```
        改之前（一个碰撞盒）                改之后（17 块凸块）
   ·   ·   ·   ·   ·   ·   ·        ·  44  44  44  49  85  97   ·
   ...
   整个占地上方一律 97 cm          座面 44–47 cm、靠背 85–98 cm
   —— 那是一块 97 cm 高的实心砖     —— 是一把真椅子
```

再横着打一条（座面下方 26 cm，模拟「椅子底下通不通」）：一个碰撞盒时撞在 0.383 m，
换成凸块之后**什么都没打到**——射线从椅腿之间穿过去了。

| 六把真碰撞餐椅 | 大客厅：单椅与茶几也是 |
|---|---|
| ![餐厅](docs/images/apt2/V3-餐厅-六把真碰撞餐椅.png) | ![大客厅](docs/images/apt2/V2-大客厅-三开间落地窗.png) |

**代价不是问题**：31 件全上 = 467 个 geom、编译 9 ms、**383 倍实时**。
而且 `nmeshgraph` 与件数**无关**（网格按资产共享），所以 8 把同款餐椅几乎免费。
